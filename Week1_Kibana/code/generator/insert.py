...
def main():
    ...
    '''
    sys.argv[1] : index prefix
    sys.argv[2] : 데이터 개수
    sys.argv[3] : index postfix
    '''
    ...
    try:
        postfix = sys.argv[3]
        mail_list = ['{}_'.format(prefix) + x.split('@')[0] + '_{}'.format(postfix) for x in mails]
    except IndexError:
        mail_list = ['{}_'.format(prefix) + x.split('@')[0] for x in mails]
    ...
    print('-'*70)
    print('총 {}개의 데이터가 입력되었습니다.'.format(len(mail_list)))
    print('-'*70)
